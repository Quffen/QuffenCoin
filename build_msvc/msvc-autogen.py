...
def parse_config_into_qfn_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Quffen Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/quffen_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/quffen_config.h'), "w", encoding="utf8") as qfn_config:
        qfn_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='Quffen-core msbuild configuration initialiser.')
    ...
    parse_config_into_qfn_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/quffen_config.h'), os.path.join(SOURCE_DIR, 'config/quffen-config.h'))
