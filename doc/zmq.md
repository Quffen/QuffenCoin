...
The Quffen Core daemon can be configured to act as a trusted "border
router", implementing the quffen wire protocol and relay, making
...
The ZeroMQ feature in Quffen Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building quffend:
...
For instance:
    ...
    $ quffend -zmqpubhashtx=tcp://127.0.0.1:215841 \
               -zmqpubhashtx=tcp://192.168.1.2:215841 \
               -zmqpubhashblock="tcp://[::1]:215842" \
               -zmqpubrawtx=ipc:///tmp/quffend.tx.raw \
               -zmqpubhashtxhwm=10000

Each PUB notification has a topic and body, where the header
corresponds to the notification type. For instance, for the
notification `-zmqpubhashtx` the topic is `hashtx` (no null
terminator). These options can also be provided in quffen.conf.
...
From the perspective of quffend, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into quffend directly. Furthermore, no information is
...
using. Quffend appends an up-counting sequence number to each
