# CJDNS support in Quffen Core

It is possible to run Quffen Core over CJDNS, an encrypted IPv6 network that
...
done outside of Quffen Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Quffen network and individual nodes.
...
before it will work with your Quffen Core node. This is described in steps
...
## Run Quffen Core with CJDNS

Once you are connected to the CJDNS network, the following Quffen Core
...
When enabled, this option tells Quffen Core that it is running in an
...
IPv6 local network. This helps Quffen Core perform better address management:
...
CJDNS support was added to Quffen Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `quffen-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/quffen/quffen/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Quffen Core

There are several ways to see your CJDNS address in Quffen Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `quffen-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `quffen-cli getpeerinfo`).
