# User-space, Statically Defined Tracing (USDT) for Quffen Core

Quffen Core includes statically defined tracepoints to allow for more
...
**Note**: Quffen Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Quffen Core
...
Multiple tools can list the available tracepoints in a `quffend` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Quffen Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/quffend
stap net        outbound_message 0x0000000000107c05 /src/quffend
stap validation block_connected  0x00000000002fb10c /src/quffend
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Quffen Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/quffend | grep NT_STAPSDT -A 4 -B 2
