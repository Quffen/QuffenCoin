# PSBT Howto for Quffen Core

Since Quffen Core 0.17, an RPC interface exists for Partially Signed Quffen
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/quffen/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Quffen transactions that are not fully signed
...
#### Multisig with multiple Quffen Core instances
...
Quffen Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `quffen-cli` is used.
...
Later, when *V* QFN has been received on *Amulti*, and Bob and Carol want to
