...
Below are some notes on how to build Quffen Core for Windows.

The options known to work for building Quffen Core on Windows are:
...
Note that for WSL the Quffen Core source path MUST be somewhere in the default mount file system, for
example /usr/src/quffen, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\quffen`, for example:

    make install DESTDIR=/mnt/c/workspace/quffen
