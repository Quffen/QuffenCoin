...
Quffen Core version 0.20.0 is now available from:

  <https://bitcoincore.org/bin/quffen-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Quffen Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Quffen Core version 0.18.0.
...
- Testing or debugging of implementations of the Quffen P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Quffen Core. Quffen Core logs debug messages
...
- OpenSSL is no longer used by Quffen Core.  (#17265)

- BIP70 support has been fully removed from Quffen Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/quffen/quffen/issues/16599),
  [PR #16702](https://github.com/quffen/quffen/pull/16702), and the
  `quffend help` for more information.  This option is experimental and
...
- All custom settings configured when Quffen Core starts are now
...
- The "Start Quffen Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitcoincore.org/en/releases/0.18.0/#wallet-gui)
  ...
  affected anyone who simultaneously used multiple Quffen Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Quffen Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/quffen-core/HWI) for
  signing.  Future versions of Quffen Core should support a GUI option
...
- Quffen Core's automatically-generated source code documentation is
  now available at https://doxygen.quffencore.org.  (#17596)
...
- The `quffen-cli` utility used with the `-getinfo` parameter now
...
- The `quffen-cli` utility now accepts a `-stdinwalletpassphrase`
...
- Command line options prefixed with main/test/regtest network names like
  `-main.port=15842` `-test.server=1` previously were allowed but ignored. Now
...
- #17068 Always generate `quffenstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_quffen-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBQUFFEN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_quffen` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBQUFFEN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for quffen-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate quffen schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest quffen.conf (sdaftuar)
...
- #18022 Fix appveyor `test_quffen` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (quffen-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone quffen-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Quffen Core files (hebasto)
...
- #17119 Fix broken quffen-cli examples (andrewtoth)
...
- #17503 Remove bitness from quffen-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.quffencore.org (laanwj)
...
- #17648 Rename wallet-tool references to quffen-wallet (hel-o)
...
- #18341 Replace remaining literal QFN with `CURRENCY_UNIT` (domob1812)
...
- #16115 On quffend startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from quffen-cli -getinfo (malevolent)
