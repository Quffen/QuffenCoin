...
TMPDIR_PREFIX = "quffen_func_test_"
...
class QuffenTestMetaClass(type):
    """Metaclass for QuffenTestFramework.

    Ensures that any attempt to register a subclass of `QuffenTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'QuffenTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("QuffenTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("QuffenTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class QuffenTestFramework(metaclass=QuffenTestMetaClass):
    """Base class for a quffen test script.

    Individual quffen test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave quffends and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop quffends after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use quffen-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_quffend = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "quffend" + config["environment"]["EXEEXT"],
        )
        fname_quffencli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "quffen-cli" + config["environment"]["EXEEXT"],
        )
        fname_quffenutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "quffen-util" + config["environment"]["EXEEXT"],
        )
        self.options.quffend = os.getenv("QUFFEND", default=fname_quffend)
        self.options.quffencli = os.getenv("QUFFENCLI", default=fname_quffencli)
        self.options.quffenutil = os.getenv("QUFFENUTIL", default=fname_quffenutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: quffends were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("QuffenRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'quffend', self.options.quffend) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'quffen-cli', self.options.quffencli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a quffend test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple quffend test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as quffend's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    quffend=self.options.quffend,
                    quffen_cli=self.options.quffencli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in quffen.conf
    ...
    def skip_if_no_quffend_tracepoints(self):
        """Skip the running test if quffend has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("quffend has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_quffend_zmq(self):
        """Skip the running test if quffend has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("quffend has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if quffen-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("quffen-wallet has not been compiled")

    def skip_if_no_quffen_util(self):
        """Skip the running test if quffen-util has not been compiled."""
        if not self.is_quffen_util_compiled():
            raise SkipTest("quffen-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if quffen-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("quffen-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether quffen-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether quffen-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_quffen_util_compiled(self):
        """Checks whether quffen-util was compiled."""
        return self.config["components"].getboolean("ENABLE_QUFFEN_UTIL")
