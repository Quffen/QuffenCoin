...
# TestP2PConn: A peer we use to send messages to quffend, and store responses.
...
class CompactBlocksTest(QuffenTestFramework):
    ...
    # This test actually causes quffend to (reasonably!) disconnect us, so do this last.
    ...
    # Test that quffend requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for quffend to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        node = self.nodes[0]
        # quffend will not send blocktxn responses for blocks whose height is
