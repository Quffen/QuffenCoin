...
`test/functional/test_framework/test_framework.py:QuffenTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_quffen_cli.py --legacy-wallet',
    'interface_quffen_cli.py --descriptors',
...
def main():
    ...
    enable_quffend = config["components"].getboolean("ENABLE_QUFFEND")

    if not enable_quffend:
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control):
    ...
    # Warn if quffend is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "quffend"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a quffend process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
...
class RPCCoverage():
    ...
    commands per `quffen-cli help` (`rpc_interface.txt`).
