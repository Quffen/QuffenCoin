...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing quffen-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='quffen-cli', help='quffen-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Quffen address to which the faucet should send')
...
parser.add_argument('quffen_cli_args', nargs='*', help='Arguments to pass on to quffen-cli (default: -signet)')

args = parser.parse_args()

if args.quffen_cli_args == []:
    args.quffen_cli_args = ['-signet']


def quffen_cli(rpc_command_and_params):
    argv = [args.cmd] + args.quffen_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = quffen_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = quffen_cli(['getnewaddress', 'faucet', 'bech32'])
