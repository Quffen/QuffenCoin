...
""" Example logging Quffen Core mempool events using the mempool:added,
...
def main(quffend_path):
    quffend_with_usdts = USDT(path=str(quffend_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    quffend_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    quffend_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    quffend_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    quffend_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[quffend_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/quffend")
        exit(1)
