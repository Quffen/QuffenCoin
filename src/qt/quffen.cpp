...
#include <config/quffen-config.h>
#endif
...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<QuffenUnit>("QuffenUnit");
#else
    qRegisterMetaType<QuffenUnit>("QuffenUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. quffen_de.qm (shortcut "de" needs to be defined in quffen.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. quffen_de_DE.qm (shortcut "de_DE" needs to be defined in quffen.qrc)
...
static const char* qt_argv = "quffen-qt";

QuffenApplication::QuffenApplication()
...
void QuffenApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the QuffenApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", QuffenGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void QuffenApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool QuffenApplication::createOptionsModel(bool resetSettings)
{
...
void QuffenApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new QuffenGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &QuffenGUI::quitRequested, this, &QuffenApplication::requestShutdown);
...
void QuffenApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void QuffenApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool QuffenApplication::baseInitialize()
{
    return node().baseInitialize();
}

void QuffenApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &QuffenApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &QuffenApplication::handleRunawayException);
    connect(this, &QuffenApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &QuffenApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void QuffenApplication::parameterSetup()
{
...
void QuffenApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void QuffenApplication::requestInitialize()
{
...
void QuffenApplication::requestShutdown()
{
...
void QuffenApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // quffen: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &QuffenGUI::handlePaymentRequest);
            connect(window, &QuffenGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void QuffenApplication::handleRunawayException(const QString &message)
{
...
void QuffenApplication::handleNonFatalException(const QString& message)
{
...
WId QuffenApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool QuffenApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", QuffenGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(quffen);
    Q_INIT_RESOURCE(quffen_locale);
    ...
    /// 6-7. Parse quffen.conf, determine network, switch to network specific
    ...
    // quffen: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
