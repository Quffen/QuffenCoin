...
/** Call node.updateRwSetting() with Quffen 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Quffen 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/quffen/quffen/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because quffen 22.x will not try to read these.
...
//! Convert enabled/size values to quffen -prune setting.
...
//! Get pruning enabled value to show in GUI from quffen -prune setting.
...
//! Get pruning size value to show in GUI from quffen -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayQuffenUnit")) {
        settings.setValue("DisplayQuffenUnit", QVariant::fromValue(QuffenUnit::QFN));
    }
    QVariant unit = settings.value("DisplayQuffenUnit");
    if (unit.canConvert<QuffenUnit>()) {
        m_display_quffen_unit = unit.value<QuffenUnit>();
    } else {
        m_display_quffen_unit = QuffenUnit::QFN;
        settings.setValue("DisplayQuffenUnit", QVariant::fromValue(m_display_quffen_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if quffen.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_quffen_unit);
...
{
    if (new_unit.isNull() || new_unit.value<QuffenUnit>() == m_display_quffen_unit) return;
    m_display_quffen_unit = new_unit.value<QuffenUnit>();
    QSettings settings;
    settings.setValue("DisplayQuffenUnit", QVariant::fromValue(m_display_quffen_unit));
    Q_EMIT displayUnitChanged(m_display_quffen_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/quffen/quffen/pull/8273
    ...
    // (https://github.com/quffen-core/gui/issues/567).
