...
#include <qt/quffenaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Quffen address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new QuffenAddressEntryValidator(parent));
    widget->setCheckValidator(new QuffenAddressCheckValidator(parent));
}
...
bool parseQuffenURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no quffen: URI
    if(!uri.isValid() || uri.scheme() != QString("quffen"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!QuffenUnits::parse(QuffenUnit::QFN, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseQuffenURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseQuffenURI(uriInstance, out);
}

QString formatQuffenURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("quffen:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(QuffenUnits::format(QuffenUnit::QFN, info.amount, false, QuffenUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openQuffenConf()
{
    ...
    /* Open quffen.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Quffen.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Quffen (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Quffen (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for Quffen*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "quffen.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("quffen-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a quffen.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Quffen\n";
        else
            optionFile << strprintf("Name=Quffen (%s)\n", chain);
