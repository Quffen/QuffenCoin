...
const std::string QuffenGUI::DEFAULT_UIPLATFORM =
...
QuffenGUI::QuffenGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &QuffenGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &QuffenGUI::showModalOverlay);
...
void QuffenGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Quffen address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and quffen: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &QuffenGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &QuffenGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &QuffenGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Quffen addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Quffen addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Quffen Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Quffen Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a quffen: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Quffen command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &QuffenGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &QuffenGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &QuffenGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &QuffenGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &QuffenGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &QuffenGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &QuffenGUI::setCurrentWallet, Qt::QueuedConnection);
                    activity->open(path);
                });
            }
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &QuffenGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &QuffenGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &QuffenGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &QuffenGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &QuffenGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &QuffenGUI::showDebugWindow);
}

void QuffenGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void QuffenGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &QuffenGUI::setCurrentWalletBySelectorIndex);
...
void QuffenGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &QuffenGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &QuffenGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &QuffenGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &QuffenGUI::showProgress);
...
#ifdef ENABLE_WALLET
void QuffenGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void QuffenGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &QuffenGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &QuffenGUI::removeWallet);
...
WalletController* QuffenGUI::getWalletController()
{
    return m_wallet_controller;
}

void QuffenGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &QuffenGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &QuffenGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &QuffenGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &QuffenGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &QuffenGUI::incomingTransaction);
    connect(this, &QuffenGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void QuffenGUI::removeWallet(WalletModel* walletModel)
{
...
void QuffenGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void QuffenGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void QuffenGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void QuffenGUI::setWalletActionsEnabled(bool enabled)
{
...
void QuffenGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &QuffenGUI::toggleHidden);
...
void QuffenGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void QuffenGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void QuffenGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void QuffenGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void QuffenGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void QuffenGUI::openClicked()
{
...
void QuffenGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void QuffenGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void QuffenGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void QuffenGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void QuffenGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void QuffenGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void QuffenGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void QuffenGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Quffen network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void QuffenGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void QuffenGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void QuffenGUI::updateHeadersSyncProgressLabel()
{
...
void QuffenGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void QuffenGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &QuffenGUI::quitRequested);
...
void QuffenGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void QuffenGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void QuffenGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &QuffenGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &QuffenGUI::show);
                e->ignore();
            }
        }
    }
...
void QuffenGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void QuffenGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void QuffenGUI::incomingTransaction(const QString& date, QuffenUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(QuffenUnits::formatWithUnit(unit, amount, true));
...
void QuffenGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void QuffenGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool QuffenGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool QuffenGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void QuffenGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void QuffenGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void QuffenGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void QuffenGUI::updateProxyIcon()
{
    std::string ip_port;
...
void QuffenGUI::updateWindowTitle()
{
...
void QuffenGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void QuffenGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void QuffenGUI::detectShutdown()
{
...
void QuffenGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void QuffenGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(QuffenGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = QuffenGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void QuffenGUI::subscribeToCoreSignals()
{
...
void QuffenGUI::unsubscribeFromCoreSignals()
{
...
bool QuffenGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<QuffenUnit> units = QuffenUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const QuffenUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, QuffenUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const QuffenUnit u : QuffenUnits::availableUnits()) {
        menu->addAction(QuffenUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(QuffenUnit newUnits)
{
    setText(QuffenUnits::longName(newUnits));
}
