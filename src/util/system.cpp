...
const char * const QUFFEN_CONF_FILENAME = "quffen.conf";
const char * const QUFFEN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //quffen-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", QUFFEN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Quffen
    // macOS: ~/Library/Application Support/Quffen
    // Unix-like: ~/.quffen
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Quffen";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Quffen";
#else
    // Unix-like
    return pathRet / ".quffen";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", QUFFEN_CONF_FILENAME));
}
